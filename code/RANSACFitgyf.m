...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    num = 0;
    for i = 1 : maxIter
        [match1,match2] = part(match, seedSetSize);
        H = ComputeAffineMatrix(p1(match1(:, 1), :), p2(match1(:, 2), :));
        dists = ComputeError(H, p1, p2, match2);
        in_dists = dists(dists<=maxInlierError);
        if size(in_dists,1) >= goodFitThresh
           new_match = [match1;match2(dists<=maxInlierError)];
           if size(in_dists,1) > num
               H = ComputeAffineMatrix(p1(new_match(:,1),:),p2(new_match(:,2),:));
               num = size(in_dists,1);
           end
        end
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    dists = ones(size(match,1),1);
    our_result = H*[pt1(match(:,1),:)';ones(1,size(match,1))];
    error = pt2(match(:,2),:)-our_result(1:2,:)';
    for i=1:size(match,1)
        dists(i,1) = sqrt(error(i,1)^2+error(i,2)^2);
    end
